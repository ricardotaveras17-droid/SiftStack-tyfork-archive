...
- `dod` / `datesOfDeath[].dod` - date of death (compare to the obituary/filing date; see DOD conflict below)
- `relativesSummary[]` - **the heir graph.** Each entry: `firstName/lastName`, `dob` (masked day, e.g. `9/XX/1955`), `relativeLevel` (**`ab` = closest kin / children**; `ac`/`ad`/`ae` = grandchildren, cousins, in-laws), `isDeceased`
- `addresses[]`, `phoneNumbers[]`

> **Gotcha:** Enformion ALWAYS returns an `error` object (`{inputErrors:[], warnings:[]}`) even on a successful search. Detect failure by HTTP status / `isError`, NOT by the presence of `error`.
...
REQUIRED SIGNERS = relativesSummary where
    relativeLevel == "ab"              (closest kin = children)
    AND lastName  == decedent surname  (children typically share the surname)
    AND isDeceased == false            (living)
    AND birth year is present          (needed for Step C lookup)
```
...
- **Rule:** Reject obituary matches where DOD is more than 3 years before the notice filing date
- **Rationale:** Probate is typically filed within 1-2 years of death; 3 years gives margin for delayed filings
- **Also reject:** DOD that is after the notice filing date (future death is impossible)
