#!/usr/bin/env python3
# DEPRECATED IN v5. The Enformion PERSON search is no longer the heir engine.
# Retained only as a reference for anyone still running deep-prospecting v4.
# Measured failure modes that retired it: zero relatives on 6 of 12 live owners,
# no phone numbers on the relatives graph (every relative is another billed
# search, 4.9x the SmartSkip cost for the same coverage), a ~50-relative cap
# that silently truncates, a surname gate that drops married-out daughters, and
# isDeceased flags that lag reality.
# Use scripts/smartskip_trace.py instead. For ENTITY owners (LLC/trust) see
# references/entity-owners-enformion-business.md, which is the one Enformion
# call v5 keeps.
